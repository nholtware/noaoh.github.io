...
const DesktopNav: FC<{navSections: SectionType[]}> = memo(({navSections}) => {
  const baseClass =
...
const MobileNav: FC<{navSections: SectionType[]}> = memo(({navSections}) => {
  ...
  return (
    <>
      <button
        ...
        className="fixed top-2 right-2 z-40 rounded-md bg-bright-red p-2 ring-offset-gray-800/60 hover:bg-red-400 focus:outline-none focus:ring-0 focus-visible:ring-2 focus-visible:ring-bright-red focus-visible:ring-offset-2 sm:hidden"
        onClick={toggleOpen}>
...
const NavItem: FC<{
  ...
  const href = section.isOnBlog
    ? section.urlName
    : section.isOnHomepage
    ? `/#${section.urlName}`
    : `/${section.urlName}`;

  return (
    <NavLink
      className={({isActive}) => classNames(isActive ? activeClass : inactiveClass)}
      key={section.urlName}
      onClick={onClick}
      to={href}>
      {section.displayName}
